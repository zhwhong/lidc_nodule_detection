...
class CT_Image(object):
    ...
    def __init__(self, input_folder, lable_file, zoomrate=1.0):
        ...
        self.train = random.sample(self.patients, int(len(self.patients)*0.7))
	...
	img = self.process_image(patient)
        ...
        #return img.reshape([1, self.depth, self.row, self.col])
	...
	train_list = random.sample(self.train, num)
        ...
        for patient in train_list:
            batch_train.append(self.padding_image(patient))
...
class CNN_3D(object):
	...
	# initialize input layer
        depth, row, col = ctobj.depth, ctobj.row, ctobj.col
	xs = tf.placeholder(tf.float32, [None, depth, row, col])
...
if __name__ == '__main__':
    ...
    ctobj = CT_Image(images_path, lable_path, zoomrate=0.125)
    cnnobj = CNN_3D(ctobj)
    cnnobj.run()
