...
def build_lstm_inner(H, lstm_input):
    ...
    # lstm_cell = rnn_cell.BasicLSTMCell(H['lstm_size'], forget_bias=0.0, state_is_tuple=True, reuse=False)
    if H['num_lstm_layers'] > 1:
        # lstm = rnn_cell.MultiRNNCell([lstm_cell] * H['num_lstm_layers'], state_is_tuple=False)
        # lstm = rnn_cell.MultiRNNCell([lstm_cell for _ in range(H['num_lstm_layers'])], state_is_tuple=True)
        lstm = rnn_cell.MultiRNNCell([rnn_cell.BasicLSTMCell(H['lstm_size'], forget_bias=0.0, state_is_tuple=True, reuse=False)  for _ in range(H['num_lstm_layers'])], state_is_tuple=True)
        # lstm = rnn_cell.MultiRNNCell([rnn_cell.BasicLSTMCell(H['lstm_size'], forget_bias=0.0, state_is_tuple=False, reuse=False)  for _ in range(H['num_lstm_layers'])], state_is_tuple=False)
    else:
        lstm = lstm_cell
