...
class PyLungError(Exception):
    """Error that will be throwed by all pylung functions"""
    pass
...
def _load_lib():
    ...
    # DMatrix functions
    lib.PyLungGetLastError.restype = ctypes.c_char_p
    return lib


# version number
__version__ = libinfo.__version__
# library instance of pylung
_LIB = _load_lib()

# type definitions
pl_uint = ctypes.c_uint
pl_float = ctypes.c_float
pl_float_p = ctypes.POINTER(pl_float)
pl_real_t = np.float32
...
def check_call(ret):
    ...
    """
    if ret != 0:
        raise PyLungError(py_str(_LIB.MXGetLastError()))
...
def ctypes2numpy_shared(cptr, shape):
    ...
    """
    if not isinstance(cptr, ctypes.POINTER(pl_float)):
        raise RuntimeError('expected float pointer')
    size = 1
    for s in shape:
        size *= s
    dbuffer = (pl_float * size).from_address(ctypes.addressof(cptr.contents))
...
def _notify_shutdown():
    """Notify PyLung about a shutdown."""
    check_call(_LIB.PyLungNotifyShutdown())
